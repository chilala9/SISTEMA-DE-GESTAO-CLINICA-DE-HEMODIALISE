<?php
include '../../config/listarPrescricao.php';
...
echo "$logados"; ?></p>
                    ...
                    <table class="table text-nowrap mb-0 align-middle">
                      <thead class="text-dark fs-4">
                        <tr>
                          ...
                          <th class="border-bottom-0">
                            <h6 class="fw-semibold mb-0">Medicamentos</h6>
                          </th>
                          <th class="border-bottom-0">
                            <h6 class="fw-semibold mb-0">Fluxo Sangue</h6>
                          </th>
                          <th class="border-bottom-0">
                            <h6 class="fw-semibold mb-0">Tempo Diálise</h6>
                          </th>
                          <th class="border-bottom-0">
    ...
    if ($prescricao1->num_rows > 0) {
        while ($listL = $prescricao1->fetch_assoc()) {
           

            echo '<tr>';
            echo "<td class='border-bottom-0'><h6 class='fw-semibold mb-0'>{$listL['id_prescricao']}</h6></td>";
            echo "<td class='border-bottom-0'>";
            echo "  <h6 class='fw-semibold mb-1'>{$listL['nome']}</h6>";

            echo "<td class='border-bottom-0'>";
            echo "<h6 class='fw-semibold mb-0 fs-4'>{$listL['dataInicio']}</h6>";
            echo '</td>';
            echo "<td class='border-bottom-0'>";
            echo "<h6 class='fw-semibold mb-0 fs-4'>{$listL['horaInicio']}</h6>";
            echo '</td>';
            echo "<td class='border-bottom-0'>";
            echo "<h6 class='fw-semibold mb-0 fs-4'>{$listL['medicamentos']}</h6>";
            echo '</td>';
            echo "<td class='border-bottom-0'>";
            echo "<h6 class='fw-semibold mb-0 fs-4'>{$listL['fluxo_sangue']}</h6>";
            echo '</td>';
            echo "<td class='border-bottom-0'>";
            echo "<h6 class='fw-semibold mb-0 fs-4'>{$listL['tempo_dialise']}</h6>";
            echo '</td>';

            if ($listL['estado_medico'] == 1) {
                echo "<td class='border-bottom-0'>";

                echo "<span class='badge bg-primary rounded-3 fw-semibold'>Atendido</span>";
                echo '</td>';
            }
        ...
        echo '<h3>Não existe  registrado</h3>';
    }
